...
int main()
{
    ...
    for(int i=1;i<256;i++)
    {
        inv_sBox[i/16][i%16]=multiplicativeInverseOverGF(sBox[i/16][i%16]);
    }
    cout << "\n\nInverse S Box is:" << endl;
    print(inv_sBox);

    bitset<8> c(string("01100011"));
    for(int i=0;i<256;i++)
    {
        for(int j=0;j<8;j++)
        {
            sBox[i/16][i%16][j] = inv_sBox[i/16][i%16][j]^inv_sBox[i/16][i%16][(j+4)%8]^
            inv_sBox[i/16][i%16][(j+5)%8]^inv_sBox[i/16][i%16][(j+6)%8]^
            inv_sBox[i/16][i%16][(j+7)%8]^c[j];
        }
    }
